        ...
        for url in urlData:        
            ...
            for page in range(totalPages):
                ...
                for jobID in offerIds:
                    ...
                    isBlacklistCompany = False
                    lowercaseJobCompany = jobCompany.casefold()
                    for company in config.blacklist:
                        if company.casefold() in lowercaseJobCompany:
                            isBlacklistCompany = True
                            break

                    isBlacklistTitle = False
                    lowercaseJobTitle = jobTitle.casefold()
                    for title in config.blacklistTitles:
                        lowercaseTitle = title.casefold()
                        if lowercaseTitle in lowercaseJobTitle:
                            isBlacklistTitle = True
                            break

                    isWrongFormat = not re.search(config.onlyApplyTitleFormat, jobTitle, re.IGNORECASE)
                    
                    jobProperties = "* | " + jobTitle +  " | " + jobCompany + " | " + jobLocation + " | " + jobWorkPlace + " | " + jobPostedDate + " | " + jobApplications
                    if isBlacklistCompany:
                        lineToWrite = jobProperties + " | " + "* 🥵 Blacklisted company!"
                        self.displayWriteResults(lineToWrite)
                        continue
                    elif isBlacklistTitle:
                        lineToWrite = jobProperties + " | " + "* 🥵 Blacklisted title!"
                        self.displayWriteResults(lineToWrite)
                        continue
                    elif isWrongFormat:
                        lineToWrite = jobProperties + " | " + "* 🥵 Job title format is wrong!"
                        self.displayWriteResults(lineToWrite)
                        continue

                    countJobs += 1
                    jobProperties = str(countJobs) + " | " + jobTitle +  " | " + jobCompany + " | " + jobLocation + " | " + jobWorkPlace + " | " + jobPostedDate + " | " + jobApplications
            ...
            prYellow("Category: " + urlWords[0] + ", " + urlWords[1] + " applied: " + str(countApplied) + " jobs out of " + str(countJobs) + ".")
